...
async def chat_endpoint(
    ...
    async def event_stream():
        try:
            # State tracker for the thinking UI box
            ...
            async for event in app_graph.astream_events(
                ...
                # Get the name of the node currently executing (e.g., 'grade_documents', 'generate_answer')
                ...
                # --- 2. STREAM LLM CONTENT ---
                    ...
                    if chunk:
                        # 1. Get the Answer Text
                        content = chunk.content
                        
                        # 2. Get the Thought Text
                        thought_token = ""
                        
                        if hasattr(chunk, "additional_kwargs") and chunk.additional_kwargs:
                            thought_token = chunk.additional_kwargs.get("reasoning_content", "") or \
                                            chunk.additional_kwargs.get("thinking", "")
                        
                        # --- STATE MACHINE ---
                        
                        # Case 1: We received a Thought Token
                        if thought_token:
                            if not in_think_block:
                                yield "THINKING_START\n"
                                in_think_block = True
                            
                            yield f"THINKING:{thought_token}\n"

                        # Case 2: We received Content (The Answer)
                        if content:
                            if in_think_block:
                                yield "THINKING_END\n"
                                in_think_block = False
                ...
                # --- 3. HANDLE TOOL CALLS ---
                    ...
                    # Safety check: If model stops but left the think block open
                    if in_think_block:
                        yield "THINKING_END\n"
                        in_think_block = False
