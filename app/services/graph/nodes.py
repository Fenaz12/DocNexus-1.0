...
if settings.LLM_HOST == "OPENROUTER": 
    ...
    response_model = ChatOllama(model=settings.OLLAMA_MODEL, reasoning=True, temperature=0.6)
    grader_model = ChatOllama(model=settings.OLLAMA_MODEL,  temperature=0).with_structured_output(GradeDocuments)
...
async def generate_answer(state: RAGState):
    ...
    messages_to_send.append(
        SystemMessage(content=f"Here is the retrieved context data for the last question:\n{context}")
    )
