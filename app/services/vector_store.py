...
class VectorStoreService:
    def __init__(self):
        ...
        if settings.LLM_HOST == "OPENROUTER": 
            self.embeddings = OpenAIEmbeddings(
                api_key=settings.OPEN_ROUTER_API,
                base_url=settings.OPEN_ROUTER_BASE_URL,
                model=settings.OPEN_ROUTER_EMBEDDING_MODEL)
            
        elif settings.LLM_HOST == "OLLAMA": 
            self.embeddings = OllamaEmbeddings(model=settings.EMBED_MODEL_ID)

        self.uri = settings.MILVUS_URI
        self.db_name = "milvus_rag_db"
    ...
    def _get_milvus_instance(self):
        connections.connect(uri=self.uri, db_name=self.db_name)
        search_params = [{ 
            # Dense
            "params": {"ef": 64}
        },
    ...
    def delete_file_chunks(self, user_id: str, file_id: int):
        """
        Deletes all chunks associated with a specific file_id and user_id.
        This is crucial when a user deletes a file or re-uploads a new version.
        """
        try:
            print(f"🗑️ Deleting chunks for file_id={file_id} (User: {user_id})...")
            ...
            # 2. Define Delete Expression
            # Milvus uses a SQL-like 'expr' to find entities to delete
            expr = f'user_id == "{user_id}" && file_id == {file_id}'
            
            # 3. Execute Delete
            # Result usually contains the number of deleted entities (depending on version)
            result = collection.delete(expr)
            
            # Force commit (optional but good for immediate consistency)
            collection.flush()
            
            print(f"✅ Deleted chunks for file_id={file_id}. Result: {result}")
